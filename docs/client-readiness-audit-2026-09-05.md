...
**可确认的前端问题已修复并通过完整回归；当前仍不能标为“全站、全用户、所有功能可正式交付”。** 既有两项真实后端运行问题、四项消费合同缺口，本次新增的一项汇总／明细口径不一致，以及尚未完成的真实角色覆盖和全站语言迁移仍影响该结论。
...
- 合并此前尚未上线的 `codex/production-function-audit` 修复，并解决与最新 main 的代码、三语言资源冲突，再增加本轮发现的七项修复（F15–F21）。
...
| F19（续查新增） | 设置页将本地草稿与服务端状态分开。保存通知偏好不再清除账户草稿；迟到的保存响应不再覆盖用户随后输入的内容 | 两种失败先复现，再通过失败→重试、跨页签草稿、迟到响应和刷新持久化回归 |
| F20（续查新增） | 分组工作区在任何写入及后续读取期间统一禁用表单控件；防止重复删除、重复退出及同时提交相互影响的成员操作 | 删除／退出按钮原先在延迟请求中仍可点击；修复后拒绝重复提交，503 后恢复可重试，并通过 390px 布局检查 |
| F21（续查新增） | 教师最近活动按响应的课程时区解析时间，已有偏移的时间戳保留其真实瞬间；用共享语言格式化显示带时区的具体日期时间 | 真实 `15:36:24 + Asia/Shanghai` 事件被线上显示为“14 小时后”；候选代码通过上海、洛杉矶、台北视区，三语言、显式偏移、刷新及无效输入回归 |
...
本轮累计记录 **433 条 HTTP 响应**：账号和读取扫描 328 条、后续业务请求 72 条、会话检查 20 条、词汇业务／退出记录 13 条。其中续查新增 5 条只读请求（4 条核对待处理请求汇总，1 条读取教学活动）；初轮为 428 条。这个数字包含正常拒绝和诊断请求，不是 433 个功能通过。交互客户端额外的建会话／退出、公开预检和浏览器动作未混入统计。

11 份消费合同共有 432 个操作条目、去重后 423 个方法／路径；本轮累计记录匹配 100 个不同操作（105 个合同条目）。未匹配的操作逐项保留为未实测，不推断其成功或失败。
...
Student 线上 Dashboard 及续查 Counsellor Dashboard 在实际 390px 视口测得 clientWidth=scrollWidth=390；临时视口已重置。全角色及其他断点来自本地 fixture 回归，未写成全角色真实 Prod 手机验收。

初轮后半程浏览器工具出现 CDP 超时，原写作标签页失去响应；当时后续点击不能取得可信的完成状态，没有据此推断产品导航故障。续查恢复了真实交互，并验证 Student 退出→Counsellor 登录、目录翻页和退出→Advisor 登录、学生搜索、计划详情／任务筛选、已发布报告和退出→Instructor 登录、身份恢复、课程目录、课次、只读报告、名单筛选和退出。最后停留在 Login 页。续查没有重新提交学习记录或管理写入，也不代表候选代码已在 Prod 验收；详见 [真实浏览器续查](evidence/client-readiness-20260905/resume-browser-acceptance.json)。

续查发现的 Advisor 汇总／请求明细不一致单列为 N1，且已对照此前清单去重。最近活动时间错误则由前端直接修复，不列为后端问题。
...
| `npm run test:run` | 163 个文件，813 项通过 |
| `npm run build` | 通过；保留现有大 chunk 提示，没有依赖／lockfile／环境值变化 |
| `npm run test:e2e` | 321 项通过，零失败；使用独立 4262 端口与模拟 API |
| `git diff --check` | 通过 |
...
- [新增 N1 汇总与明细响应](evidence/client-readiness-20260905/resume-hub-count.json)
- [教学活动真实时区响应](evidence/client-readiness-20260905/resume-activity-time.json)
- [续查真实浏览器验收](evidence/client-readiness-20260905/resume-browser-acceptance.json)
