...
4. **Some design projections still differ.** Calendar uses the supported combined date/time controls rather than the reference's separate date/duration model. Course/observer content uses the fields permitted by that role. Course reader capture covers a LINK item; video/PDF-specific content and multi-item next navigation need additional acceptance evidence. Structured person names replace unsupported nickname fields. Exact variant-by-variant pixel parity remains unaccepted.
