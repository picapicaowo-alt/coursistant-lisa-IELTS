...
const syntheticPassword = () => `Aa1!${randomUUID()}`;
...
const OUT = '/tmp/xlearn-resumed-visual-review';
  ...
  return async (state: string, nodes: string[], note = '') => {
    ...
    for (const width of [1440, 390]) {
      ...
      if (width === 390 && ['student-reading-questions', 'student-reading-results', 'student-writing-exam', 'student-writing-results'].includes(state)) {
        ...
        const exit = await navigation.getByRole('button', {name: 'Exit', exact: true}).boundingBox(); await fs.writeFile(path.join(OUT, `${state}-navigation-check.json`), JSON.stringify({rect, exit, metrics: await navigation.evaluate(element => ({client: element.clientHeight, scroll: element.scrollHeight, top: element.scrollTop, documentTop: window.scrollY, documentHeight: document.documentElement.scrollHeight, focused: document.activeElement?.tagName}))}, null, 2)); expect(exit && exit.y >= 0 && exit.y + exit.height <= 845).toBeTruthy();
...
async function richFixture(page: Page, role = 'STUDENT') {
  ...
  await page.route('**/v2/student/mock-exams?*', route => route.fulfill({json: reply({items: exams, page: 0, size: 20, total: exams.length})}));
...
test('capture student profile settings and crop states', async ({page}) => {
  ...
  await page.getByLabel('Current password', {exact: true}).fill(syntheticPassword()); await page.getByLabel('New password', {exact: true}).fill(newPassword); await page.getByLabel('Confirm new password', {exact: true}).fill(newPassword); await page.locator('form button[type=submit]').click(); await expect(page.getByText('Password updated.', {exact: true})).toBeVisible(); await capture('settings-password-success', ['406:3008']);
});
...
test('capture advisor directory journey and course dialogs', async ({page}) => {
  ...
  const courses = ['Academic Writing Studio', 'Speaking with Confidence', 'Critical Reading'].map((title, index) => ({courseId: 71 + index, courseCode: `EN-${101 + index}`, title, deliveryMode: 'GROUP', instructorFirstName: 'Ivy', instructorLastName: 'Lee', status: 'PUBLISHED', courseLinkVersion: 1, lectureCompleted: 4, lectureTotal: 10, schedule: [{sessionId: 11, type: 'LECTURE', dayOfWeek: 'MONDAY', startTime: '10:30:00', endTime: '12:00:00', location: 'Room 3A'}]}));
...
test('capture exam library writing and completed states', async ({page}) => {
  ...
  await page.goto('/mock-exams'); await expect(page.getByText('IELTS Academic Practice 1', {exact: true})).toBeVisible(); await capture('student-exam-library', ['163:698', '423:3034'], 'Actual assigned and completed exam cards; timed-sitting dates are absent from the assignment contract.');
...
test('capture authentication form transition states', async ({page}) => {
  ...
  await page.getByLabel('Email', {exact: true}).fill('review@example.test'); await page.getByLabel('Password', {exact: true}).fill(syntheticPassword()); await page.locator('button[type=submit]').click(); await expect(page.getByRole('alert')).toBeVisible(); await capture('auth-login-error', ['729:3484']);
  ...
  await page.locator('#signup-email').fill('review@example.test'); await page.locator('#signup-tenant-id').fill('1'); await page.locator('button[type=submit]').click(); await capture('auth-signup-details', ['730:4753'], 'Required structured names and tenant registration contract differ from the Figma activation form.'); await page.locator('#signup-first-name').fill('Alex'); await page.locator('#signup-last-name').fill('Chen'); await page.locator('#signup-password').fill(newPassword); await page.locator('#signup-confirm-password').fill(newPassword); await page.locator('button[type=submit]').click(); await expect(page.locator('#signup-verification')).toBeVisible(); await capture('auth-signup-verification', ['730:4753'], 'Verification code is contract-required; no legal-consent contract is consumed.');
  ...
  await page.getByLabel('Digit 1').fill('123456'); await page.locator('button[type=submit]').click(); await expect(page.locator('#reset-password')).toBeVisible(); await capture('auth-reset-password', ['732:4924']); await page.locator('#reset-password').fill(newPassword); await page.locator('#reset-password-confirm').fill(newPassword); await page.locator('button[type=submit]').click(); await expect(page.locator('#reset-password')).toHaveCount(0); await capture('auth-reset-success', ['732:4973']);
});
...
test('capture resumed student dashboard', async ({page}) => {
  await richFixture(page); const capture = recorder(page, 'dashboard');
  await page.goto('/'); await expect(page.getByRole('heading', {name: /Welcome back/})).toBeVisible(); await expect(page.getByText('IELTS Academic Practice 1', {exact: true})).toBeVisible();
  await capture('student-dashboard', ['17:914', '108:882', '466:3289'], 'Live-contract fixture dashboard; persisted playback, AI history and activity series remain contract gaps.');
});
