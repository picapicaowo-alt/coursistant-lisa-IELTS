...
| Configured delivery hid Instructor week authoring, although the same Instructor successfully created a week through the documented API | Separate week management from course administration; keep role and archive restrictions. |
...
| Course preparation | Created QA group course; documented preconfiguration session POST succeeded; delivery configuration and readiness validation succeeded | Publication was rejected even after readiness reported READY with no blockers; investigation remains open |
...
| P1 — class dates and new course publication | `GET /v2/courses/{courseId}/session-occurrences?from=…&to=…&includeHistory=false`; Advisor delivery/launch operations | Advisor delivery config returns 200 for a QA course; occurrence read returns 404 `COURSE_NOT_FOUND`; readiness returns READY with no blockers, publish displays “Course is not ready” | Reconcile course visibility and publish readiness; repeat after course content publication; a real Instructor week creation already succeeded on the configured course |
