...
// Known codes use centralized labels; authored reasons and unknown diagnostic codes stay intact.
const readableReason = (reason: string) => /^[A-Z_]+$/.test(reason) ? statusLabel(reason) : reason;
...
export function AdvisorDashboardOverview({name, dashboard, students, tasks, conversations, schedule, loading, error, onRetry}: {
  ...
  return <>
    <header className={styles.welcome}>
      <div className={styles.greeting}><UserAvatar className={styles.welcomeAvatar}/><h1>{translate("advising:overview.welcome", {name})}</h1></div>
      <div className={styles.summary}>{dashboard.stats.slice(0, 4).map(stat => <span key={stat.key} data-stat={stat.key}>{stat.label} <strong>{loading || error ? '—' : formatNumber(stat.value)}</strong></span>)}</div>
    </header>
    {error ? <p role="alert" className={styles.error}>{translate("advising:overview.partialFailed")}{' '}<button type="button" onClick={onRetry}>{translate("common:actions.retry")}</button></p> : null}
    <div className={styles.grid}>
      <div className={styles.column}>
        <WorkspaceSection title={translate("advising:overview.attention")} meta={<ViewAll to={APP_ROUTE_PATHS.advisorStudents}/>} bodyClassName={styles.attentionBody}>
          {attention.length === 0 ? <p className={styles.empty}>{loading ? translate("advising:overview.loadingStudents") : translate("advising:overview.noAttention")}</p> : attention.slice(0, 5).map((student, index) => <div className={styles.attentionRow} key={student.studentUserId}>
            <div className={styles.person}><UserAvatar userId={student.studentUserId} className={styles.avatar}/><div><strong>{formatPersonName(student, translate('common:people.studentFallback', {id: formatNumber(student.studentUserId)}))}</strong><small>{student.targetGoal || translate("advising:overview.noGoal")}</small></div></div>
            <AdvisingBadge value={student.highestPriority}/>
            <div className={styles.reason}><span>{student.riskReasons?.[0] ? readableReason(student.riskReasons[0]) : translate("assistant:prompts.progress")}</span>{student.riskReasons && student.riskReasons.length > 1 ? <small>{student.riskReasons.slice(1).map(readableReason).join(' · ')}</small> : null}</div>
        ...
        <WorkspaceSection title={translate("advising:overview.todayTasks")} meta={<ViewAll to={APP_ROUTE_PATHS.advisorTasks}/>} bodyClassName={styles.taskBody}>
          ...
          {tasks.length === 0 ? <p className={styles.empty}>{loading ? translate("dashboard:loadingTasks") : translate("advising:overview.noTasks")}</p> : tasks.slice(0, 4).map(task => {
            ...
            return <div className={styles.taskRow} key={task.taskId}>
              ...
              <Link className={styles.taskTitle} to={task.taskId == null ? APP_ROUTE_PATHS.advisorTasks : `${APP_ROUTE_PATHS.advisorTasks}?taskId=${task.taskId}`}><strong>{task.description || translate("advising:overview.reviewTask")}</strong><small>{formatPersonName(students.find(student => student.studentUserId === task.studentUserId), task.studentUserId ? translate('common:people.studentFallback', {id: formatNumber(task.studentUserId)}) : translate("common:tasks.advisingTask"))}</small></Link>
      ...
      <div className={styles.column}>
        ...
        <WorkspaceSection title={translate("advising:overview.messages")} meta={<ViewAll to={APP_ROUTE_PATHS.advisorMessages}/>} bodyClassName={styles.messagesBody}>
          {conversations.length === 0 ? <p className={styles.empty}>{loading ? translate("learning:messages.loading") : translate("advising:overview.noConversations")}</p> : conversations.slice(0, 5).map(conversation => <Link className={styles.messageRow} data-unread={conversation.unreadCount > 0 || undefined} to={`${APP_ROUTE_PATHS.advisorMessages}?studentUserId=${conversation.studentUserId}`} key={conversation.studentUserId}>
            <UserAvatar userId={conversation.studentUserId} className={styles.avatar}/><div><strong>{conversation.studentName}</strong><small>{conversation.latestPreview || translate("advising:overview.startConversation")}</small></div>
            {conversation.unreadCount > 0 ? <span className={styles.unread} aria-label={translate('advising:overview.unread', {count: conversation.unreadCount, number: formatNumber(conversation.unreadCount)})}>{formatNumber(conversation.unreadCount)}</span> : conversation.latestAt ? <time dateTime={conversation.latestAt}>{formatUtcTimestamp(conversation.latestAt)}</time> : null}
          </Link>)}
        </WorkspaceSection>
        {schedule.length > 0 ? <Link className={styles.requestSummary} to={APP_ROUTE_PATHS.advisorSchedule}><span><strong>{translate("operations:scheduleRequests")}</strong><small>{translate("advising:overview.awaitingReview", {number: formatNumber(schedule.filter(request => request.status === 'PENDING').length)})}</small></span><ChevronRight size={18}/></Link> : null}
      </div>
    </div>
  </>;
}
...
function TaskCheckbox({task, pending, onChange}: {task: AdvisorActionTaskResponse; pending: boolean; onChange: () => void}) {
  ...
  const label = task.status === 'PENDING' ? translate("advising:studentTasks.start") : task.status === 'IN_PROGRESS' ? translate("advising:actionTasks.resolve") : translate("common:status.COMPLETED");
  return <label className={styles.taskCheck}><input ref={input} type="checkbox" checked={task.status === 'RESOLVED'} disabled={pending || task.version == null || task.taskId == null || !['PENDING', 'IN_PROGRESS'].includes(task.status ?? '')} aria-label={translate("advising:overview.taskAction", {action: label, task: task.description || translate("advising:overview.actionTask")})} onChange={onChange}/></label>;
}
