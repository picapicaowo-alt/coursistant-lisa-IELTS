...
type DashboardStates = Record<'dashboard' | 'students' | 'tasks' | 'conversations' | 'schedule', QueryState>;

function CollectionStatus({state, subject, empty}: {state: QueryState; subject: string; empty: string}) {
  return <p className={styles.empty} role={state.isError ? 'alert' : 'status'}>{
    state.isPending ? `Loading ${subject}…` : state.isError ? `Could not load ${subject}.${isHttpStatus(state.error, 403) || isHttpStatus(state.error, 404) ? ' Access is not available for this account.' : ' Use Retry above to try again.'}` : empty
  }</p>;
}
...
export function AdvisorDashboardOverview({name, dashboard, students, tasks, conversations, schedule, states, onRetry}: {
  ...
  schedule: AdvisorScheduleRequestView[]; states: DashboardStates; onRetry: () => void;
}) {
  const transition = useActionTaskTransition();
  const error = Object.values(states).some(state => state.isError);
  const retryable = Object.values(states).some(state => state.isError && !isHttpStatus(state.error, 403) && !isHttpStatus(state.error, 404));
  const metricsUnavailable = states.dashboard.isPending || states.dashboard.isError;
  ...
  const total = progress.reduce((sum, stat) => sum + (stat.value ?? 0), 0);
  return <>
    <header className={styles.welcome}>
      <div className={styles.greeting}><UserAvatar className={styles.welcomeAvatar}/><h1>Welcome back, {name}!</h1></div>
      <div className={styles.summary}>{dashboard.stats.slice(0, 4).map(stat => <span key={stat.key} data-stat={stat.key}>{stat.label} <strong>{metricsUnavailable ? '—' : stat.value ?? '—'}</strong></span>)}</div>
    </header>
    {error ? <p role="alert" className={styles.error}>Some dashboard information could not be loaded. {retryable ? <button type="button" onClick={onRetry}>Retry</button> : null}</p> : null}
    <div className={styles.grid}>
      ...
      <div className={styles.column}>
        <WorkspaceSection title="Need Attention" meta={<ViewAll to={APP_ROUTE_PATHS.advisorStudents}/>} bodyClassName={styles.attentionBody}>
          {attention.length === 0 ? <CollectionStatus state={states.students} subject="students" empty="No students currently need attention."/> : attention.slice(0, 5).map((student, index) => <div className={styles.attentionRow} key={student.studentUserId}>
            <div className={styles.person}><UserAvatar userId={student.studentUserId} className={styles.avatar}/><div><strong>{formatPersonName(student, `Student #${student.studentUserId}`)}</strong><small>{student.targetGoal || 'Goal not set'}</small></div></div>
        ...
        <WorkspaceSection title="Action Tasks" meta={<ViewAll to={APP_ROUTE_PATHS.advisorTasks}/>} bodyClassName={styles.taskBody}>
          ...
          {tasks.length === 0 ? <CollectionStatus state={states.tasks} subject="action tasks" empty="No action tasks to show."/> : tasks.slice(0, 4).map(task => {
            // Creation/start timestamps are not deadlines. The current contract has no due time.
            const createdAt = task.createdAt && !Number.isNaN(Date.parse(task.createdAt)) ? task.createdAt : undefined;
            return <div className={styles.taskRow} key={task.taskId}>
              ...
              <AdvisingBadge value={task.category} kind="category"/>
              <span className={styles.taskTime}>{createdAt ? <><small>Created</small><time dateTime={createdAt}>{formatUtcTimestamp(createdAt)}</time></> : null}</span>
            </div>;
          })}
        </WorkspaceSection>
        <WorkspaceSection title="Progress Overview" meta={<span className={styles.period}>Current caseload</span>}>
          {/* The contract supplies current counts, not week/month score history. */}
          {!metricsUnavailable && progress.every(stat => stat.value != null) ? <div className={styles.progressBar} aria-label="Student risk distribution">{progress.map(stat => <span key={stat.key} data-stat={stat.key} style={{flexGrow: stat.value}} title={`${stat.label}: ${stat.value}`}/>)}{total === 0 ? <span/> : null}</div> : null}
          <dl className={styles.stats}>{progress.map(stat => <div key={stat.key} data-stat={stat.key}><dt>{stat.label}</dt><dd>{metricsUnavailable ? '—' : stat.value ?? '—'}</dd></div>)}</dl>
        </WorkspaceSection>
      </div>
      <div className={styles.column}>
        ...
        <WorkspaceSection title="Recent Messages" meta={<ViewAll to={APP_ROUTE_PATHS.advisorMessages}/>} bodyClassName={styles.messagesBody}>
          {conversations.length === 0 ? <CollectionStatus state={states.conversations} subject="messages" empty="No recent conversations."/> : conversations.slice(0, 5).map(conversation => <Link className={styles.messageRow} data-unread={conversation.unreadCount > 0 || undefined} to={`${APP_ROUTE_PATHS.advisorMessages}?studentUserId=${conversation.studentUserId}`} key={conversation.studentUserId}>
        ...
        {states.schedule.isError ? <CollectionStatus state={states.schedule} subject="schedule requests" empty=""/> : null}
