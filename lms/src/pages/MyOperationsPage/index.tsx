...
import {StudentWorkQueue, StudentScheduleRequests} from './StudentQueues';
  ...
  return (
    <div className={embedded ? styles.embedded : styles.page}>
      ...
      {section === 'overview' && student ? <div className={styles.workspace}>
        ...
        <div className={styles.grid}><StudentWorkQueue/><StudentScheduleRequests key={`requests-${courseId}`} courseId={validCourse ? selectedCourseId : undefined}/></div>
        <StudentReports key={`reports-${courseId}`} courseId={validCourse ? selectedCourseId : undefined}/>
        {validCourse ? <WorkspaceSection title="Purchased course hours"><QueryFeedback pending={hours.isPending} error={hours.error} onRetry={() => void hours.refetch()}/>{hours.isSuccess ? <RecordSummaryList value={hours.data} emptyMessage="No purchased-hours record is available."/> : null}</WorkspaceSection> : <section className={styles.emptyPanel}><strong>Select a course for course hours and schedule changes</strong><span>Choose one of your enrolled courses above.</span></section>}
      ...
      {[{label: 'Course selection', query: courses}, {label: 'Course hours', query: hours}, {label: 'Calendar', query: calendar}, {label: 'Personal events', query: personalEvents}].filter(({query}) => query.isError).map(({label, query}) => <div className={styles.inlineAlert} key={label} role="alert"><p>{getApiErrorMessage(query.error, `${label} could not be loaded.`)}</p><button type="button" onClick={() => void query.refetch()}>Retry {label.toLowerCase()}</button></div>)}
