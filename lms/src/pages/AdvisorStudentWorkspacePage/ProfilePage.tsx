  ...
  return (
    <section className={styles.card}>
      <WorkspaceSectionHeader
        ...
        meta={!missing ? <span className={styles.versionBadge}>Version {query.data?.profileVersion}</span> : undefined}
      />
      ...
      <form className={styles.form} onSubmit={onSubmit}>
        ...
        <section className={styles.formSection}>
          ...
          {form.skills.map((skill, index) => (
            <fieldset key={`${skill.position}-${index}`} className={styles.recordGroup}>
          ...
          <button type="button" className={styles.secondary} onClick={() => setForm(current => ({...current, skills: [...current.skills, emptySkill(current.skills.length + 1)]}))}>Add another skill</button>
        </section>
        <section className={styles.formSection}>
        ...
        <div className={styles.formActions}><button className={styles.primary} disabled={save.isPending}>{save.isPending ? 'Saving…' : missing ? 'Create profile' : 'Save profile'}</button></div>
      </form>
    </section>
  );
};
