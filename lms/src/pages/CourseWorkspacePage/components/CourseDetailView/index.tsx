    ...
    <div className={`${styles.learningWorkspace} ${instructorView ? styles.instructorWorkspace : ''}`}>
      <header className={styles.learningHeader}>
        <div><h1 className={styles.courseTitle}>{course.title || course.name || course.courseCode}</h1>{course.description ? <p className={styles.description}>{course.description}</p> : null}
          <div className={styles.courseContext}><div><span>Course code</span><strong>{course.courseCode}</strong></div>{canViewOwnGrades ? <AssignmentProgress progress={studentProgress.data?.courses?.find(item => item.courseId === course.id)} loading={studentProgress.isFetching} failed={studentProgress.isError}/> : null}<div><span>Instructor</span><strong>{formatInstructorName(course.primaryInstructor, 'Not assigned')}</strong></div><div><span>Term</span><strong>{[course.termStartDate, course.termEndDate].filter(Boolean).join(' – ') || 'Not provided'}</strong></div>{instructorView ? <TeachingBadge value={course.state}>{course.state}</TeachingBadge> : <div><span>Course status</span><strong>{course.state}</strong></div>}</div>
        </div>
