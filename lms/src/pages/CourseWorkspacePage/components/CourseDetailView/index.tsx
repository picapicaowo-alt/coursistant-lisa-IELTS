    ...
    <div className={`${styles.learningWorkspace} ${instructorView ? styles.instructorWorkspace : styles.studentWorkspace}`}>
      <header className={styles.learningHeader}>
        <div><h1 className={styles.courseTitle}>{course.title || course.name || course.courseCode}</h1>{course.description ? <p className={styles.description}>{course.description}</p> : null}
          <div className={styles.courseContext}><div><span>Course code</span><strong>{course.courseCode}</strong></div><div><span>Instructor</span><strong>{formatInstructorName(course.primaryInstructor, 'Not assigned')}</strong></div><div><span>Term</span><strong>{[course.termStartDate, course.termEndDate].filter(Boolean).join(' – ') || 'Not provided'}</strong></div>{instructorView ? <TeachingBadge value={course.state}>{course.state}</TeachingBadge> : <div><span>Course status</span><strong>{course.state}</strong></div>}</div>
        </div>
        {activeTab === 'courses' ? <section className={styles.currentLesson} aria-label="Selected course content"><img src="/icons/figma-dashboard/study-plan.svg" alt=""/><div><span>Course content</span><h2>{activeWeek?.title || 'No content published yet'}</h2><p>{activeWeek?.materials.length ?? 0} learning {activeWeek?.materials.length === 1 ? 'material' : 'materials'}</p></div>{activeWeek ? <a href={`#week-${activeWeek.id}`} className={styles.addButton} onClick={() => {setActiveTab('courses'); setActiveWeekId(activeWeek.id);}}>Open learning materials</a> : null}</section> : null}
      </header>
