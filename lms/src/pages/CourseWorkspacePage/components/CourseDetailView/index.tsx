        ...
        <div className={styles.cards}>
          {activeTab === 'courses' ? <>
            {weeks.length === 0 ? <p className={styles.cardEmpty}>No course materials have been published yet.</p> : weeks.map((week, index) => <section className={styles.learningUnit} key={week.id} id={`week-${week.id}`}>
              <button type="button" className={styles.unitHeader} aria-expanded={activeWeekId === week.id} aria-controls={`week-content-${week.id}`} onClick={() => setActiveWeekId(current => current === week.id ? null : week.id)}><span className={styles.unitNumber}>{index + 1}</span><span><small>{week.materials.length} materials · {week.state}</small><strong>{week.title}</strong></span><img src="/icons/figma-dashboard/arrow-right.svg" alt=""/></button>
              {activeWeekId === week.id ? <div id={`week-content-${week.id}`}><ContentCard week={week} onOpenMaterial={openMaterial}/></div> : null}
            </section>)}
