...
Tenant Administration extends the existing X-Learn operational workspace: calm, precise enterprise operations with low information density, generous whitespace, clear record hierarchy, and a visible next action. Cool-gray working space, white outlined surfaces, inherited HarmonyOS Sans TC typography, and violet wayfinding express the supplied Tenant Admin frames without creating a separate brand.
...
- **Policy groups:** the user's follow-up reference supersedes the accordion layout. Three policy modes precede one airy eight-row list. Numeric rules open a focused editor drawer; only the three contracted check flags have switches. A shared mode explanation replaces repeated unavailable notices. Edits apply to a local draft before an explicit whole-policy save; the baseline version stays pinned. A pending switch to system mode never relabels prior custom values as platform defaults. See `docs/tenant-alert-rules-low-density-2026-09-03.md` for the refinement boundary.
