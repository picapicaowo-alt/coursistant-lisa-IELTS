  ...
  return (
    <main className={styles.page}>
      ...
      {editorOpen ? <form className={styles.card} onSubmit={submit}>
        ...
        {sessions.isPending ? <p className={styles.muted}>Loading schedule…</p> : sessions.isError ? <p className={styles.error}>Could not load the schedule.</p> : ordered.length === 0 ? <p className={styles.muted}>No class times have been set.</p> : <ul className={styles.eventList}>{ordered.map(session => <li key={session.id}><div className={styles.announcementRow}><div className={styles.sessionSummary}><span className={styles.dateTile}><strong>{session.dayOfWeek}</strong></span><span className={styles.eventText}><strong>{session.type}</strong><small><Clock3 size={13}/> {session.startTime.slice(0, 5)} – {session.endTime.slice(0, 5)} · {dayLabel(session.dayOfWeek)}{session.location ? <> · <MapPin size={13}/> {session.location}</> : null}</small></span></div>{canEditSchedule ? <div className={styles.rowActions}><button type="button" className={styles.iconButton} aria-label={`Edit ${session.type} on ${dayLabel(session.dayOfWeek)}`} onClick={() => { setDraft(toDraft(session)); setEditingId(session.id); setEditorOpen(true); }}><Pencil size={16}/></button>{confirmDeleteId === session.id ? <><button type="button" className={styles.dangerButton} onClick={() => remove.mutate(session.id)}>Confirm</button><button type="button" className={styles.secondaryButton} onClick={() => setConfirmDeleteId(null)}>Cancel</button></> : <button type="button" className={styles.iconButton} aria-label={`Delete ${session.type} on ${dayLabel(session.dayOfWeek)}`} onClick={() => setConfirmDeleteId(session.id)}><Trash2 size={16}/></button>}</div> : null}</div></li>)}</ul>}
      </section>}
    </main>
  );
};
