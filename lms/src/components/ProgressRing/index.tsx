...
export function ProgressRing({value, label, inverse = false}: {value: number | null; label: string; inverse?: boolean}) {
  const progress = value == null ? null : Math.max(0, Math.min(100, Math.round(value)));
  return (
    <div className={styles.ring} data-inverse={inverse || undefined}
