...
export function AdvisorInstructorPicker({value, onChange, label = 'Instructor', required = false}: {
  value: string;
  ...
  return <fieldset className={styles.form}>
    <legend>{label}</legend>
    <label>Search instructors<input type="search" maxLength={100} value={q} onChange={event => {setQ(event.target.value); setPage(0);}}/></label>
    <label>{label}<select required={required} value={value} onChange={event => onChange(event.target.value)}>
      ...
      {instructors.data?.items.map(item => <option key={item.instructorUserId} value={item.instructorUserId}>{formatPersonName(item)} · {item.email}</option>)}
    </select></label>
