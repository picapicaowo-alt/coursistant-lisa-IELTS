...
| `CKV2_AWS_20`, `CKV_AWS_2`, `CKV_AWS_103` | Conditional public ALB listeners | The reusable module can create a temporary HTTP listener only when no certificate is supplied. The live pilot has `enable_https = true`, an issued certificate, TLS 1.2+, and HTTP redirect. | Remove the temporary listener path when all future environments can provision DNS before their first apply. |
...
| `CKV_AWS_31` | Valkey AUTH | The replication group uses TLS plus `auth_token_wo`; a 64-character ephemeral token is written to Secrets Manager without entering plan or state. | Adopt a supported automatic rotation workflow and validate zero-downtime client reconnection. |
| `CKV_AWS_18`, `CKV_AWS_144`, `CKV_AWS_145`, `CKV2_AWS_62` | Frontend origin and access-log buckets | The origin logs to a dedicated terminal log bucket. The log bucket uses SSE-S3 for CloudFront delivery compatibility and has no recursive logging or cross-region replication. | Add the approved DR region and central immutable log archive before production. |
| `CKV_AWS_158` | Global WAF log group | A second customer-managed KMS key in `us-east-1` is deferred for the pilot; authorization headers are redacted and retention is one year. | Add a dedicated `us-east-1` log KMS key and update the WAF logging destination. |
| `CKV_AWS_310`, `CKV_AWS_374` | CloudFront | A single private S3 origin is sufficient for the pilot, and the product is intentionally global rather than geo-blocked. WAF managed rules and rate limiting remain active. | Define frontend DR/origin failover and revisit geographic controls using the approved market and compliance policy. |
