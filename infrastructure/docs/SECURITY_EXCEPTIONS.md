...
| `CKV2_AWS_20`, `CKV_AWS_2`, `CKV_AWS_103` | Public ALB | The initial endpoint remains HTTP only while the externally hosted DNS validation CNAME is pending. WAF and rate limiting remain active. | Validate the Tokyo ACM certificate, set `enable_https = true`, and verify the HTTP listener only redirects to TLS 1.2+. |
